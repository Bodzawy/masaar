...
export const metadata = { title: "Scheduled Lessons" };

const STATUS_LABEL: Record<string, { label: string; variant: "success" | "warning" | "muted" | "destructive" | "default" }> = {
  SCHEDULED: { label: "Scheduled", variant: "default" },
  IN_PROGRESS: { label: "In progress", variant: "warning" },
  COMPLETED: { label: "Completed", variant: "success" },
  CANCELLED_BY_STUDENT: { label: "Cancelled by you", variant: "muted" },
  CANCELLED_BY_TEACHER: { label: "Cancelled by teacher", variant: "destructive" },
  ...
  return (
    <div className="container max-w-4xl space-y-8 animate-fade-in">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Scheduled Lessons</h1>
          <p className="mt-1 text-sm text-muted-foreground">Your live one-to-one sessions.</p>
        </div>
        <Button asChild><Link href="/student/teachers">Book a lesson</Link></Button>
      </header>
      ...
      <section>
        ...
        {upcoming.length === 0 ? (
          <Card><CardContent className="flex flex-col items-center p-10 text-center">
            ...
            <p className="mt-3 font-medium">No lessons scheduled</p>
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              Book an instant lesson with an online teacher or schedule a session for later.
            </p>
            <Button size="sm" className="mt-4" asChild><Link href="/student/teachers">Find a teacher</Link></Button>
          </CardContent></Card>
        ) : (
                ...
                <Card>
                  <CardContent className="flex flex-wrap items-center gap-4 p-4">
                    <div className="min-w-[140px]">
                      <p className="font-semibold">{new Date(b.scheduledAt).toLocaleString("en-GB", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</p>
                    ...
                    <div className="flex gap-2">
                      <Button size="sm" asChild><Link href={`/student/classroom/${b.id}`}><Video aria-hidden /> Classroom</Link></Button>
      ...
      <section>
        ...
        {past.length === 0 ? (
          <p className="rounded-lg border border-dashed border-border p-6 text-sm text-muted-foreground">No past lessons yet.</p>
        ) : (
          <ul className="space-y-2">
            {past.map((b) => (
              <li key={b.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-border bg-card px-4 py-3">
                <span className="w-32 shrink-0 text-xs text-muted-foreground">{new Date(b.scheduledAt).toLocaleDateString("en-GB")}</span>
                ...
                {b.rating ? (
                  ...
                  <Button size="sm" variant="outline" asChild><Link href={`/student/post-lesson/${b.id}`}>Rate</Link></Button>
                ) : null}
              </li>
            ))}
          </ul>
...
function CancelForm({ bookingId }: { bookingId: string }) {
  return (
    ...
    >
      <Button type="submit" size="sm" variant="ghost" className="text-destructive hover:text-destructive">Cancel</Button>
    </form>
  );
}
