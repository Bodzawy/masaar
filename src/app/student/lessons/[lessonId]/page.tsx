  ...
  const tasks = [
    {
      key: "attendance",
      icon: BookOpenCheck,
      title: "Live-Unterricht besuchen",
      desc: "Die Teilnahme wird automatisch nach der Stunde erfasst.",
    ...
    },
    {
      key: "quiz",
      icon: QuizIcon,
      title: `Quiz bestehen (${PROGRESSION.lessonQuizPassPercent}%+)`,
      desc: progress.quizBestScore != null ? `Bisheriges Ergebnis: ${progress.quizBestScore}%` : "20 Fragen · Grammatik, Wortschatz, Lesen und Hören.",
      done: progress.quizPassed,
      cta: { label: progress.quizBestScore != null && !progress.quizPassed ? "Quiz wiederholen" : "Quiz starten", href: `/student/lessons/${lesson.id}/quiz` },
    ...
    ...(lesson.requiresHomework
      ? [{
          key: "homework",
          icon: PenLine,
          title: "Hausaufgabe einreichen",
          desc: "Schreibaufgabe + Übungen. Entwürfe werden gespeichert.",
          done: progress.homeworkSubmitted,
          cta: { label: "Hausaufgabe öffnen", href: `/student/homework/${lesson.id}` },
          disabled: !progress.attendanceComplete,
        }]
      : []),
  ];
  ...
  return (
    <div className="container max-w-5xl space-y-6 animate-fade-in">
      ...
      <nav className="text-sm text-muted-foreground" aria-label="Breadcrumb">
        <Link href="/student/learning-path" className="hover:text-foreground">Mein Lernweg</Link>
      ...
      <header className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={progress.status === "COMPLETED" ? "success" : "default"}>
            Lektion {lesson.orderIndex} · {lesson.chapter.title}
          </Badge>
          <Badge variant="muted" className="gap-1"><Clock className="h-3 w-3" />{lesson.durationMinutes} Min. Live-Unterricht</Badge>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="space-y-6 lg:col-span-3">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-base"><Target className="h-4 w-4 text-primary" aria-hidden /> Lernziele</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>Am besten vor oder während der Live-Stunde ansehen.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 sm:grid-cols-2">
              {lesson.materials.length === 0 && <p className="text-sm text-muted-foreground">Noch keine Materialien.</p>}
            ...
            <CardContent className="space-y-3">
              {tasks.map((task) => (
                <div key={task.key} className={`flex flex-wrap items-center gap-3 rounded-lg border p-4 ${task.done ? "border-success/40 bg-success/5" : task.disabled ? "border-border opacity-70" : "border-border"}`}>
                  ...
                  {task.done ? (
                    <CheckCircle2 className="h-5 w-5 text-success" aria-label="Abgeschlossen" />
                  ) : task.disabled ? (
                    <span className="flex items-center gap-1.5 text-xs text-muted-foreground"><Lock className="h-3.5 w-3.5" aria-hidden /> Zuerst Unterricht besuchen</span>
                  ) : (
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card>
            <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><Users2 className="h-4 w-4 text-primary" aria-hidden /> Live-Unterricht</CardTitle></CardHeader>
            <CardContent>
              {upcomingBooking ? (
                <div className="space-y-2 text-sm">
                  <p className="font-medium">{upcomingBooking.teacher.name}</p>
                  <p className="text-xs text-muted-foreground">{new Date(upcomingBooking.scheduledAt).toLocaleString("de-DE")}</p>
                ...
                <div className="space-y-3 text-sm">
                  <p className="text-muted-foreground">Für diese Lektion ist noch keine Stunde gebucht.</p>
                  <Button size="sm" variant="accent" className="w-full" asChild>
                    <Link href={`/student/teachers?lesson=${lesson.id}`}>Jetzt Lehrkraft finden</Link>
                  </Button>
                </div>
              )}
            </CardContent>
          ...
          <Card className="bg-muted/50">
            <CardContent className="p-4 text-xs leading-relaxed text-muted-foreground">
              <strong className="text-foreground">So funktioniert das Freischalten.</strong> A lesson completes when you (1) am Live-Unterricht
              teilnehmen, (2) im Quiz mindestens {PROGRESSION.lessonQuizPassPercent}% erreichen und{" "}
              {lesson.requiresHomework ? "(3) die Hausaufgabe einreichen." : "das Quiz bestehen."} Die nächste Lektion wird
              dann automatisch freigeschaltet – manuelles Überspringen ist nicht möglich.
            </CardContent>
          </Card>
        </div>
      </div>
...
function LockedState({ reason, passScore }: { reason: string; passScore: number }) {
  ...
  return (
    <div className="container max-w-lg py-24 text-center">
      ...
      <h1 className="mt-4 text-xl font-semibold">Diese Lektion ist gesperrt</h1>
      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{messages[reason] ?? "Vorherige Schritte abschließen, um freizuschalten."}</p>
