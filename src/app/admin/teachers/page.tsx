...
export const metadata = { title: "Teachers & Onboarding" };
    ...
    <div className="container max-w-6xl space-y-8 animate-fade-in">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Pipeline: {ONBOARDING_STAGES.map((s, i) => `${i + 1}. ${s.replaceAll("_", " ").toLowerCase()}`).join(" → ")}
        </p>
      </header>

      {/* Onboarding pipeline */}
      <section>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Pipeline ({pipeline.length})</h2>
        {pipeline.length === 0 ? (
          <Card><CardContent className="p-6 text-sm text-muted-foreground">No applications in progress.</CardContent></Card>
        ) : (
              ...
              <li key={t.id}>
                <Card>
                  <CardContent className="flex flex-wrap items-center gap-4 p-4">
                    ...
                    >
                      <Button type="submit" size="sm" variant="outline">Advance stage</Button>
                    </form>
                  </CardContent>
                </Card>
              </li>
      ...
      {/* Active teachers */}
      <section>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Active teachers ({active.length})</h2>
                ...
                <CardContent className="flex h-full flex-wrap items-start gap-4 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {TEACHER_RANKS[t.rank as TeacherRankKey].label} · teaches up to {TEACHER_RANKS[t.rank as TeacherRankKey].maxLevel}
                    </p>
                    <p className="mt-1.5 text-xs text-muted-foreground">
                      {t.completedLessons.toLocaleString()} lessons · cancel rate {t.cancellationRatePct}% · response ~{t.responseTimeMinutes}m
                    </p>
                  </div>
                  <div className="text-end">
                    <Badge variant={QUALITY_VARIANT[t.qualityState]}>{t.qualityScore}/100</Badge>
                    <p className="mt-1 text-[11px] text-muted-foreground">{QUALITY_VARIANT[t.qualityState] === "success" ? "Good Standing" : t.qualityState.replaceAll("_", " ").toLowerCase()}</p>
                  </div>
                </CardContent>
              </Card>
            </li>
