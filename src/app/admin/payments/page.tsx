    ...
    <div className="container max-w-5xl space-y-6 animate-fade-in">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Zahlungen der Lernenden und Auszahlungen an Lehrkräfte (Demo-Anbieter).</p>
        </div>
        <p className="text-sm"><span className="font-semibold tabular-nums">{formatPrice(revenueCents)}</span> <span className="text-muted-foreground">eingenommen (gesamt)</span></p>
      </header>

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">Zahlungen der Lernenden</CardTitle></CardHeader>
          <CardContent>
            <ul className="space-y-1.5 text-sm">
              {payments.map((pay) => (
                <li key={pay.id} className="flex flex-wrap items-center gap-2 rounded-md px-2 py-1.5 odd:bg-muted/60">
                  ...
                  <Badge variant={pay.status === "PAID" ? "success" : pay.status === "REFUNDED" ? "muted" : pay.status === "FAILED" ? "destructive" : "warning"}>{pay.status === "PAID" ? "bezahlt" : pay.status.toLowerCase()}</Badge>
              ...
              {payments.length === 0 && <li className="text-xs text-muted-foreground">Keine Zahlungen.</li>}
            </ul>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-base">Auszahlungen an Lehrkräfte</CardTitle></CardHeader>
          <CardContent>
            <ul className="space-y-1.5 text-sm">
              {payouts.map((payout) => (
              ...
              {payouts.length === 0 && <li className="text-xs text-muted-foreground">Keine Auszahlungen.</li>}
            </ul>
          </CardContent>
        </Card>
      </div>
